...
io.on("connection", (socket) => {
    ...
    try {
      ...
      if (message.to !== "anonymous") {
        ...
        if (recipientSocket) {
          ...
          console.log(
            `File forwarded to ${message.to} (socket ${recipientSocket})`
          );
        }
      } else {
  ...
  socket.on("recallMessage", (message) => {
    ...
    try {
      const receiverSocket = users.get(message.to);
      if (!receiverSocket) {
        throw error("did not found receiver..");
      }
      io.to(receiverSocket).emit("messageRecalled", {
        id: message.id,
      });

      
    } catch (e) {
      console.error("Error while recalling:", error);
    }
  });
  ...
  socket.on("messageRead", async (messageId) => {
    ...
    try {
      ...
      if (receiver && receiver.isIncognito) {
        // console.log(
        //   `User ${receiver._id} is in incognito mode. Not emitting read status.`
        // );
        return;
      }
...
app.get("*", (req, res) => {
  res.sendFile(
    path.join(__dirname, "secure-chat-frontend", "build", "index.html")
  );
});
